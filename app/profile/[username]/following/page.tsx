...
export default function FollowingPage() {
        ...
        const searchGlobal = async () => {
            ...
            try {
                ...
                const results = (res?.data || res || []).filter((u: any) =>
                    !followingList.some(f => f.username === u.username) && u.username !== profileUsername
                );
    ...
    }, [searchQuery, followingList, profileUsername]);
    ...
    const handleUnfollow = async (username: string) => {
        ...
        try {
            await api.user.unfollow(username, token);
            setFollowingList(prev => prev.filter(f => f.username !== username));
        } catch (err) {
        ...
        <div className="flex flex-col gap-4 animate-in fade-in slide-in-from-bottom-5 duration-700 pb-20">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12 pl-6 border-l-[3px] border-[#e63030]">
                <div className="flex flex-col gap-1.5 flex-1">
                    ...
                    <p className="text-white/20 text-[10px] font-black uppercase tracking-[0.4em]">Tracking {followingList.length} signals in the Verse</p>
                </div>
                ...
                <div className="relative group w-full md:w-96 flex items-center justify-end">
                    ...
                    {!searchOpen && (
                        <button
                            ...
                            className="p-4 bg-white/[0.02] border border-white/10 hover:bg-white/[0.04] transition-all"
                        >
                            <svg className="w-5 h-5 text-white/40" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}><path strokeLinecap="round" strokeLinejoin="round" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
                        </button>
                    )}
                </div>
            </div>
