...
export default function FollowingPage() {
    ...
    useEffect(() => {
        // Fetch following list from backend if available
        // For now, using mock data as fallback until backend is updated
        const fetchData = async () => {
            setLoading(true);
            try {
                // If there was an api.user.getFollowing, we'd call it here
                // const res = await api.user.getFollowing(profileUsername);
                ...
                setFollowingList([
                    ...
                    { id: 5, name: "GojoSatoru_Fan", username: "gojo_fan", avatar: "https://avatar.iran.liara.run/public/boy?username=gojo_fan", bio: "The strongest fan of the strongest sorcerer" },
                    { id: 6, name: "MangaArtist_X", username: "manga_x", avatar: "https://avatar.iran.liara.run/public/girl?username=manga_x", bio: "Drawing my way through the anime verse" },
                ]);
            } catch (err) {
                console.error(err);
            } finally {
    ...
    const handleUnfollow = (id: number | string) => {
        setFollowingList(prev => prev.filter(f => f.id !== id));
    };
        ...
        <div className="flex flex-col gap-4 animate-in fade-in slide-in-from-bottom-5 duration-700">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12 pl-6 border-l-[3px] border-[#e63030]">
                <div className="flex flex-col gap-1.5">
                    <h2 className="text-3xl font-black uppercase tracking-[0.2em] text-white">Following</h2>
                    <p className="text-white/20 text-[10px] font-black uppercase tracking-[0.4em]">Tracking {filteredList.length} signals in the Verse</p>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                {filteredList.map((friend) => (
                    ...
                    >
                        ...
                        <div className="shrink-0 flex items-center pl-6 pr-2">
                            <button
